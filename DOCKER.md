# Hosting Spork in Docker (browser play)
...
Godot install required for players. Same shape as `../godot-rpg/DOCKER.md`.

```
Browser (Godot Web export)  ──ws://HOST:PORT/ws──►  Caddy  ──►  Godot server (:8766)
                            ◄── http (web client) ──┘
```
...
Pick a name and hit **Play** — the client auto-connects to `ws://localhost:8767/ws`.
Open a second browser/tab to join as a second player — you'll see their mini avatar
appear in the roster on the left, an "X joined the arena" toast, and their real-time
growth in the (Tab-toggled) scoreboard. Both players can turn on **PvP** and click/
spike each other in the arena.
...
The published port defaults to **8767**. Override it with `WEB_PORT`:
...
- **Cloudflare Tunnel** (no port-forwarding, trusted cert): point an ingress
  hostname at `http://<host>:8767`. Players open the tunnel's `https://` URL; the
...
| Piece | Where | Notes |
|-------|-------|-------|
| Dedicated server | `Dockerfile` target `server` | `godot --headless --audio-driver Dummy --path /game -- --server` → `menu.gd` calls `Net.start_dedicated_server()`. The server is **not** a player; it just brokers the roster and relays position/state/combat. |
| Web client | `Dockerfile` target `build` → `web` | Exported with the `Web` preset (threads off), served by Caddy from `/srv`. |
| Reverse proxy | `Caddyfile` | Plain HTTP on container `:80`; `/ws` → `server:8766` WebSocket upgrade. |
| Published port | `docker-compose.yml` | `${WEB_PORT:-8767}` → web container `:80`. |

The same `game_state.gd`/`player_state.gd`/`net.gd` code runs as before — only the
*hosting* changed. Desktop builds can still **Host Game** / **Join Game** natively
(or **Play Solo** with no networking at all).
...
- **Can't connect / WebSocket errors** — check `docker compose logs server` for the
  "Spork dedicated server listening on ws port 8766" line; confirm you're hitting the
  same `WEB_PORT` you published. If testing two clients from the same machine outside
  Docker, a local firewall can silently block loopback WebSocket connections — approve
  any Windows Security prompt, or just test through the actual Docker deployment.
